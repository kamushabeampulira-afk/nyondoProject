...
router.post("/members", isManagerOrAdmin, async (req, res) => {
  try {
    ...
    if (existing)
      throw new Error("Member with same NIN or phone already exists");
...
router.post("/pickup", isManagerOrAdmin, async (req, res) => {
  try {
    ...
    if (!memberId || !productId || !quantity || quantity < 1)
    ...
    if (product.currentStock < quantity)
      throw new Error(
        `Only ${product.currentStock} units of ${product.productName} in stock`,
      );
    const total = product.unitPrice * quantity;
    if (member.balance < total)
      throw new Error(
        `Insufficient balance. Required: ${total.toLocaleString()} UGX, Available: ${member.balance.toLocaleString()} UGX`,
      );
...
router.get("/member-statement", isManagerOrAdmin, async (req, res) => {
  try {
    ...
    if (!targetMember) {
      return res.render("member-statement", {
        member: null,
        membersList,
        user: req.user,
      });
    ...
    let transactions = await DepositTransaction.find(query).sort({
      createdAt: -1,
    });
    ...
    const lastTransactionDate = transactions.length
      ? transactions[0].createdAt.toLocaleDateString()
      : null;
