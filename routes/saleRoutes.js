...
router.post('/add', isSalesOrAdmin, async (req, res) => {
  try {
    ...
    if (product.currentStock < qty) throw new Error(`Only ${product.currentStock} of ${product.productName} in stock`);
    const cart = req.session.cart || [];
    const existing = cart.find(i => i.productId == productId);
    if (existing) existing.qty += qty;
    else cart.push({ productId, name: product.productName, price: product.unitPrice, qty });
...
router.post('/quick-checkout', isSalesOrAdmin, async (req, res) => {
  ...
  session.startTransaction();
  try {
    const { productId, quantity, customerName, distance, paymentMethod } = req.body;
    const qty = parseInt(quantity);
